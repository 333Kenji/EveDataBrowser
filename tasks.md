...
- [X] **M0-T01 Bootstrap Docker compose stack** (`docker/compose.yml`, `docker/.env.example`)
...
- [X] **M0-T02 Author multi-stage Dockerfiles** (`docker/Dockerfile.ingestion`, `docker/Dockerfile.backend`, `docker/Dockerfile.frontend`)
  ...
  - **Depends on**: M0-T01.
- [X] **M0-T03 Scaffold tooling & lint** (`pyproject.toml`, `Makefile`, `package.json`)
  ...
  - **Depends on**: M0-T02.
- [X] **M0-T04 Stub FastAPI app** (`backend/src/main.py`, `backend/src/api/health.py`, `backend/tests/test_health_smoke.py`)
...
- [X] **M0-T05 Document SDE archive handling** (`README.md`, `.gitignore`, `tasks.md` note)
...
- [X] **M1-T01 Watcher & manifest discovery** (`ingestion/src/watcher.py`, `ingestion/tests/test_watcher.py`)
...
- [X] **M1-T02 Decompression + manifest writer** (`ingestion/src/pipeline.py`, `ingestion/tests/test_manifest_pipeline.py`)
  ...
  - **Depends on**: M1-T01.
- [X] **M1-T03 Schema migrations** (`backend/src/db/migrations/V001__sde_tables.py`)
  ...
  - **Depends on**: M0-T02.
- [X] **M1-T04 Data loaders** (`ingestion/src/dogma.py`, `ingestion/src/blueprints.py`, `ingestion/tests/test_dogma_join.py`)
...
- [X] **M1-T05 Quickstart updates** (`docs/quickstart.md`)
...
- [X] **M2-T01 Design tokens** (`frontend/src/styles/tokens.ts`, `docs/ui/styleguide.md`)
  ...
  - **Depends on**: M0-T03.
- [X] **M2-T02 Sidebar navigation** (`frontend/src/components/Sidebar.tsx`, `frontend/tests/components/Sidebar.spec.tsx`)
  ...
  - **Depends on**: M0-T04.
- [X] **M2-T03 Search skeleton** (`frontend/src/components/SearchShell.tsx`, `frontend/src/hooks/useSearchParams.ts`)
  ...
  - **Depends on**: M2-T02.
- [X] **M2-T04 Background filaments** (`frontend/src/components/BackgroundWeb.tsx`, `frontend/tests/components/BackgroundWeb.spec.tsx`)
...
- [X] **M3-T01 `/search` endpoint** (`backend/src/api/search.py`, `backend/tests/test_search.py`)
...
- [X] **M3-T02 Ship repository helpers** (`backend/src/db/repository.py`, `backend/tests/test_repository_ships.py`)
  ...
  - **Depends on**: M1-T03, M1-T04.
- [X] **M3-T03 ShipCard UI** (`frontend/src/components/cards/ShipCard.tsx`, `frontend/tests/components/ShipCard.spec.tsx`)
...
- [X] **M3-T04 Optional 3D viewer** (`frontend/src/components/cards/ShipViewer.tsx`, `frontend/src/assets/ships/manifest.json`)
...
- [X] **M4-T01 Blueprint repository helpers** (`backend/src/db/repository.py`, `backend/tests/test_repository_blueprints.py`)
  ...
  - **Depends on**: M1-T03, M1-T04.
- [X] **M4-T02 `/blueprints/:id` endpoint** (`backend/src/api/blueprints.py`, `backend/tests/test_blueprints.py`)
  ...
  - **Depends on**: M4-T01.
- [X] **M4-T03 BlueprintCard UI** (`frontend/src/components/cards/BlueprintCard.tsx`, `frontend/tests/components/BlueprintCard.spec.tsx`)
...
- [X] **M4-T04 Filters integration** (`frontend/src/components/Filters.tsx`, `frontend/tests/components/Filters.spec.tsx`)
...
- [X] **M5-T01 Market schema + rate limiter** (`backend/src/db/migrations/V002__market_tables.py`, `backend/src/services/rate_limiter.py`)
  ...
  - **Depends on**: M1-T03.
- [X] **M5-T02 Provider stubs** (`ingestion/src/market/adam4eve_stub.py`, `ingestion/src/market/fuzzwork_stub.py`, `ingestion/tests/test_market_stub.py`)
  ...
  - **Depends on**: M5-T01.
- [X] **M5-T03 `/market/{type_id}` endpoint** (`backend/src/api/market.py`, `backend/tests/test_market.py`)
  ...
  - **Depends on**: M5-T02.
- [X] **M5-T04 Market chart UI** (`frontend/src/components/cards/BlueprintMarketChart.tsx`, `frontend/tests/components/BlueprintMarketChart.spec.tsx`)
